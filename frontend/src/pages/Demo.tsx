...
const validatePromptClient = (
  ...
  const unavailableSubject = UNAVAILABLE_SUBJECTS.find(
    (subj) =>
      lowerPrompt.includes(subj) &&
      !lowerPrompt.includes(subject.toLowerCase()),
  );
...
const SimulationIframe = React.memo(function SimulationIframe({
  simulationData,
}: {
...
export default function Demo(): JSX.Element {
  ...
  const isTokenLimitReached = useMemo(
    () => role !== 'dev' && tokenUsage >= TOKEN_LIMIT,
    [role, tokenUsage],
  );
  ...
  const handleRunSimulation = useCallback(
    async (inputPrompt?: string): Promise<void> => {
      ...
      try {
        ...
        if (data.error) {
          throw new Error(`Simulation error: ${data.error}`);
        }
